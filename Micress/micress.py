...
from Micress import micressConfig as mConf
...
from Micress import MICFieldID
...
class micress(Model.Model):
    ...
    def __init__(self, file='' , workdir='' ):
        ...
        """
        super(micress, self).__init__() # call base class
        '''
        super(micress, self).initialize(file,workdir)   
        '''
        ...
        self.osType = platform.system() #获取操作系统类型
        ...
        self.micressExec = mConf.micressExec #micressExec = micressPath + "/starter.sh"

        try:
            
          os.environ['ACMICRES_LICENSE_FILE'] = mConf.micressLicense #'@222.28.70.246'
          os.environ['LSHOST'] = mConf.thermocalcLicense #'@222.28.70.246'
        except:
          pass
    ...
    def setProperty(self, property, objectID=0):
        ...
        """
        if (property.getPropertyID() == MICPropertyID.PID_InterfacialEnergy):
            self.T0.append(1)
            self.T1.append(None)
            ...
            self.IE.append(None)
            self.locIndex = 0
            self.IE[self.locIndex] = property
            
        elif (property.getPropertyID() == MICPropertyID.PID_Temperature):
          if (self.locIndex != -1):
    ...
    def solveStep(self, tstep, stageID=0, runInBackground=False):
        ...
        for line in f:
          ...
          if ( line.find('<IE>') > -1 ):
            line = line.replace('<IE>',str(self.IE[self.locIndex].getValue(tstep.getTime()))+'E-05')  
        ...
        self.proc = run([self.micressExec,temporaryDrifile],stdout=self.scr,stderr=self.scr,timeout=10)
    ...
    def parseTabKfile(self):
      ...
      f = open(self.workbench + '\\' + "3Graingrowth_dri_loc_" + str(self.locIndex) + extension,'r')
